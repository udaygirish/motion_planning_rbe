...
# Custom functions written for joint intersection , orientation and coincidence check

def calculate_internal_angle(link1, link2):
    # Calculate the internal angle between two links using the dot product
    # Return degrees
    vector1 = np.array(link1[1]) - np.array(link1[0])
    vector2 = np.array(link2[1]) - np.array(link2[0])
